public class SelectDice {
  static int N; // 전체 개수
  static int max = Integer.MAX_VALUE;
  static int[] answer;
  static int[][] dices;
  static List<Integer> choice = new ArrayList<>(); // 어떤 주사위를 뽑을지
  static List<Integer> arrA;
  static List<Integer> arrB;

  // public static void main(String[] args) {
  // int[][] dice = {{1, 2, 3, 4, 5, 6}, {3, 3, 3, 3, 4, 4}, {1, 3, 3, 4, 4, 4},
  // {1, 1, 4, 4, 5, 5}};
  // int[] result = solution(dice);
  // for (int r : result) {
  // System.out.println(r + ' ');
  // }
  // }

  public int[] solution(int[][] dice) {
    N = dice.length;
    dices = dice;
    answer = new int[N / 2];
    choiceDice(0, 0);
    return answer;
  }

  static void makeArrAB() {
    arrA = new ArrayList<>();
    arrB = new ArrayList<>();

    int[][] diceA = new int[N / 2][6];
    int[][] diceB = new int[N / 2][6];

    int a = 0, b = 0;

    for (int i = 0; i < N; i++) {
      if (choice.contains(i)) {
        diceA[a] = dices[i];
        a++;
      } else {
        diceB[b] = dices[i];
        b++;
      }
    }

    makeArr(0, diceA, 0, arrA);
    makeArr(0, diceB, 0, arrB);
  }

  // dfs
  static void makeArr(int depth, int[][] dice, int sum, List<Integer> arr) {
    // N/2 만큼씩 뽑아서 더한 다음에 저장?
    if (depth == N / 2) {
      arr.add(sum);
      return;
    }
    for (int i = 0; i < 6; i++) {
      int newSum = sum + dice[depth][i];
      makeArr(depth + 1, dice, newSum, arr);
    }
  }

  static int calculateWinning() {
    int count = 0;

    makeArrAB();

    Collections.sort(arrB);

    for (int i = 0; i < arrA.size(); i++) {
      int number = arrA.get(i);

      int left = 0, right = arrB.size() - 1;

      int index = Integer.MIN_VALUE;
      while (left <= right) {
        int mid = (left + right) / 2;

        if (arrB.get(mid) < number) {
          left = mid + 1;
          index = Math.max(index, mid);
        } else {
          right = mid - 1;
        }
      }

      if (index != Integer.MIN_VALUE) {
        count += index + 1;
      }
    }

    return count;
  }

  static void choiceDice(int depth, int s) {
        if (depth == N / 2) {
            int winning = calculateWinning();
            if (max < winning) {
                max = winning;
                for (int i = 0; i < choice.size(); i++) {
                    answer[i] = choice.get(i) + 1;
                }
            }
            return;
        }
        for (int i = s; i < N; i++) {
            choice.add(i);
            choiceDice(depth + 1, i + 1);
            choice.remove(choice.size() - 1);
        }
}
